...
function bar128($text,$name) { // Part 1, make list of widths
 ...
 $onChar=1;
 for($x=0;$x<strlen($text2);$x++) // GO THRU TEXT GET LETTERS
 if (!( ($pos = strpos($char128asc,$text2[$x])) === false )){ // SKIP NOT FOUND CHARS
 $w.= $char128wid[$pos];
 $sum += $onChar++ * $pos;
 } 
 $w.= $char128wid[ $sum % 103 ].$char128wid[106]; //Check Code, then END
 //Part 2, Write rows
 $html="<table cellpadding=0 cellspacing=0 style='margin-top: 10px'><tr>"; 
